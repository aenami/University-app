...
const registrarNota = async (req, res) => {
    ...
    try {
        ...
        const id_docente = await Nota_js_1.default.getDocenteByUsuario(req.idUser);
        if (!id_docente) {
        ...
        const matricula = await Nota_js_1.default.getMatriculaByEstudianteYGrupo(id_estudiante, id_grupo);
        if (!matricula) {
        ...
        const corteExiste = await Nota_js_1.default.existeCorte(id_corte);
        if (!corteExiste) {
            return res.status(404).json({ error: true, message: "Corte no encontrado" });
        }
        // 4. Verificar que no exista nota duplicada
        const notaDuplicada = await Nota_js_1.default.existeNota(id_estudiante, id_asignatura, id_corte);
        if (notaDuplicada) {
...
const obtenerNotasPorGrupo = async (req, res) => {
    ...
    try {
        const grupoExiste = await Nota_js_1.default.existeGrupo(idGrupo);
        if (!grupoExiste) {
            return res.status(404).json({ error: true, message: "Grupo no encontrado" });
        }
        const notas = await Nota_js_1.default.getNotasByGrupo(idGrupo);
...
exports.obtenerNotasPorGrupo = obtenerNotasPorGrupo;
// ============================================================
...
const editarNota = async (req, res) => {
    ...
    try {
        ...
        const nota = await Nota_js_1.default.getNotaById(idNota);
        if (!nota) {
        ...
        // 3. Actualizar nota
        await Nota_js_1.default.actualizarNota(idNota, valor);
