...
const crearMatricula = async (req, res) => {
    const { id_estudiante, id_grupo, id_programa, id_beca, detalles } = req.body;
    // ── Validación básica ──────────────────────────────────────
    ...
    try {
        ...
        const [estudianteRows] = await connection.query(`SELECT id_estudiante FROM estudiante WHERE id_estudiante = ?`, [id_estudiante]);
        ...
        const [matriculaExistente] = await connection.query(`SELECT id_matricula FROM matricula
             WHERE id_estudiante = ? AND id_programa = ?
             LIMIT 1`, [id_estudiante, id_programa]);
        ...
        const [grupos] = await connection.query(`SELECT
                g.id_grupo,
                g.cupo_maximo,
                a.creditos,
             ...
             WHERE g.id_grupo IN (?)
             GROUP BY g.id_grupo, g.cupo_maximo, a.creditos`, [idsGrupos]);
        ...
        if (id_beca) {
            const [becaRows] = await connection.query(`SELECT descuento FROM beca
                 WHERE id_beca = ?
                   AND fecha_inicio <= CURDATE()
                   AND fecha_fin    >= CURDATE()`, [id_beca]);
            if (!becaRows.length) {
                await connection.rollback();
                res.status(404).json({
                    ok: false,
        ...
        const [resultMatricula] = await connection.query(`INSERT INTO matricula
